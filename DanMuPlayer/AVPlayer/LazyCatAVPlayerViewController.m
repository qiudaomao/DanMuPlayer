...
- (void)dealloc {
    NSLog(@"LazyCatAVPlayerViewController dealloc");
}
        ...
        if (playerItem.status == AVPlayerItemStatusReadyToPlay) {
            ...
            UIAlertAction *continueWatching = [UIAlertAction actionWithTitle:@"继续观看"
                                                                       style:UIAlertActionStyleDefault handler:^(UIAlertAction* action){
                                                                           NSLog(@"continue play from %.2f", self->currentResumeTime);
                                                                           [self seekToTime:self->currentResumeTime];
                                                                           [self->player play];
                                                                       }];
...
-(void)stop {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self->danmu) {
            [self->danmu removeFromSuperview];
        }
//        if (self->danmuView) {
  ...
  withResumeTime:(CGFloat)resumeTime {
    ...
    if (videosSource.segments.count==1) {
        ...
        if (![scheme isEqualToString:@"http"] && ![scheme isEqualToString:@"https"]) {
            ...
            resourceLoader = [[CustomResourceLoader alloc] initWithHeaders:headers scheme:@"http"];
