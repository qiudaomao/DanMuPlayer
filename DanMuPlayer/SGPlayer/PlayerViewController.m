...
//    danmuView = [[DanMuView alloc] initWithFrame:self.view.bounds];
...
- (void)progressAction:(NSNotification *)notification
{
    ...
    [self updateNowPlaying:current duration:duration];
    ...
    if (hide) {
        ...
        if (delay) {
            _hideDelayTimer = [NSTimer scheduledTimerWithTimeInterval:5.0 repeats:NO block:^(NSTimer * _Nonnull timer) {
                self->_hudInHidenProgress = YES;
                [UIView animateWithDuration:1.0f animations:^{
                    [self->hudLayer setAlpha:0.0f];
                    [self->_timeLabel setAlpha:0.0f];
                    [self->_presentSizeLabel setAlpha:0.0f];
                } completion:^(BOOL finished) {
                    [self->hudLayer setHidden:YES];
                    [self->_timeLabel setHidden:YES];
                    [self->_presentSizeLabel setHidden:YES];
                    self->_hudInHidenProgress = NO;
        ...
        case SGPlayerStateReadyToPlay:
            ...
            if ((_resumeTime > 0.0f || _realResumeTime > 0.0f) && self.player.duration > 0.0f) {
                ...
                UIAlertAction *continueWatching = [UIAlertAction actionWithTitle:@"继续观看"
                                                                           style:UIAlertActionStyleDefault handler:^(UIAlertAction* action){
                                                                               NSLog(@"continue play from %.2f", self->_resumeTime);
                                                                               CGFloat resumeTime = self->_resumeTime;
                                                                               self->_resumeTime = 0.0f;
                                                                               self->_realResumeTime = 0.0f;
...
//    [danmuView setSubTitle:subTitle_ withColor:UIColor.whiteColor withStrokeColor:UIColor.blackColor withFontSize:35];
}
