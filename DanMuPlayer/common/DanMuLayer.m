...
@interface DanMuLayer() {
    ...
    NSMutableArray<StrokeUILabel*> *cachedLabels;
}
@end
...
withStrokeColor:(UIColor*)bgcolor
   withFontSize:(CGFloat)fontSize {
    ...
    StrokeUILabel *label = nil;
    if (cachedLabels.count > 0) {
        label = cachedLabels.lastObject;
        [cachedLabels removeLastObject];
    } else {
        label = [[StrokeUILabel alloc] init];
    }
    ...
    cachedLabels = [NSMutableArray arrayWithCapacity:100];
    //NSLog(@"add width %.2f x %.2f of %@", frame.size.width, frame.origin.x, content);
    label.speed = -260.0f - 260.0f*frame.size.width/size.width;// * frame.size.width/200;
    if (dandao == nil) {
...
-(void)updateFrame {
    ...
    //dispatch_async(dispatch_get_main_queue(), ^{
        for (NSMutableArray<StrokeUILabel*> *danmus in dandao) {
            ...
            for (StrokeUILabel *label in needRemoveItems) {
                if (cachedLabels.count < 100) {
                    [cachedLabels addObject:label];
                }
            }
